...
- **重启后「界面透明」不生效**：`onload` 只走了 `applyCurrent()`（画面 + 遮罩），从未调用
  `renderer.applyUI()`。于是 panels / opacity 模式虽然已经写进 `local.json`，但重启思源后要等用户
  动一下设置页才会应用 —— 看起来就像「设置没存下来」。现在启动、外部存储变更、用户改动统一走
  同一条 `applyConfig()`：可见性 / 界面透明模式 / 画面参数 / 轮换定时器一次到位。
- **关闭状态重启时背景层未隐藏**：`enabled=false` 时启动不再套用任何配置，`#we-bg` 也就没有
  `we-off` 类。现在会正确隐藏背景层，并清除界面透明样式。
- **切换壁纸不再重复重建素材**：`applyConfig()` 会比较目标壁纸的稳定标识（`key`），
  只有目标真的变了才重新创建 video / iframe；单纯调参只更新滤镜与遮罩。
- **壁纸被移除后画面残留**：当前壁纸解析不到时清空画面层，不再留着上一次的素材。
- 关闭插件时不再重复执行 `applyCurrent()`（此前会短暂重建一次背景素材）。
